...
function setWX(options) {
    ...
    var shareUrl = options.shareLink;

    var ACTIVE_APPID = CONF_BASE[options.env].app_id;
    var ACTIVE_HOST = CONF_BASE[options.env].wx_host;
    if (MOBILE_ENV != 'WX' && options.env == "production") {
        history.replaceState('', '', ACTIVE_HOST+shareUrl);
    }
    ...
    wx.ready(function () {
        ...
        wx.onMenuShareTimeline({
            ...
            title: title, // 分享标题
            desc: desc, // 分享描述
            link: ACTIVE_HOST+shareUrl, // 分享链接
            imgUrl: imgUrl, // 分享图标
            success: callback,
            cancel: function () {
                // 用户取消分享后执行的回调函数
